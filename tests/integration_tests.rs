...
fn test_size_and_hash_comparison() {
    ...
    let has_size_only_group =
        stdout.contains("size_only1.txt") && stdout.contains("size_only2.txt");
    assert!(
        !has_size_only_group,
...
fn test_actual_deletion_with_force() {
    ...
    assert!(
        status.success(),
        "Force-delete should succeed, stderr: {}",
        stderr
    );
    assert!(file_kept.exists(), "First (delete.txt) should be kept");
    assert!(
        !file_deleted.exists(),
        "Second (keep.txt) should be deleted"
    );
}
...
fn test_min_size_filter_works_correctly() {
    ...
    let (stdout, stderr, status) =
        run_fundoubler(&[temp_dir.path().to_str().unwrap(), "--min-size=5"]);
...
fn test_max_size_filter_works_correctly() {
    ...
    let (stdout, stderr, status) =
        run_fundoubler(&[temp_dir.path().to_str().unwrap(), "--max-size=10"]);
...
fn test_sha512_hash_flag() {
    ...
    let (stdout, stderr, status) = run_fundoubler(&[temp_dir.path().to_str().unwrap(), "--sha512"]);
...
fn test_xxh3_hash_flag() {
    ...
    let (stdout, stderr, status) = run_fundoubler(&[temp_dir.path().to_str().unwrap(), "--xxh3"]);
...
fn test_content_flag_enables_all_hashes() {
    ...
    let (stdout, stderr, status) =
        run_fundoubler(&[temp_dir.path().to_str().unwrap(), "--content"]);
...
fn test_name_flag_finds_duplicates_by_name() {
    ...
    sub1.child("common.txt")
        .write_str("identical content")
        .unwrap();
    sub2.child("common.txt")
        .write_str("identical content")
        .unwrap();

    let (stdout, stderr, status) =
        run_fundoubler(&[temp_dir.path().to_str().unwrap(), "--name", "--md5"]);
...
fn test_limit_option() {
    ...
    let (stdout, stderr, status) =
        run_fundoubler(&[temp_dir.path().to_str().unwrap(), "--md5", "--limit=2"]);
...
fn test_silent_mode() {
    ...
    let (stdout, stderr, status) =
        run_fundoubler(&[temp_dir.path().to_str().unwrap(), "--md5", "--silent"]);
...
fn test_verbose_mode() {
    ...
    let (stdout, stderr, status) =
        run_fundoubler(&[temp_dir.path().to_str().unwrap(), "--md5", "--verbose"]);
...
fn test_init_config_creates_default_file() {
    ...
    let (_stdout, stderr, status) =
        run_fundoubler(&["--init-config", config_path.to_str().unwrap()]);
...
fn test_init_config_includes_all_fields() {
    ...
    let (_, stderr, status) = run_fundoubler(&["--init-config", config_path.to_str().unwrap()]);
    assert!(status.success(), "stderr: {}", stderr);
...
fn test_invalid_path() {
    ...
    let (_stdout, _stderr, status) = run_fundoubler(&["/nonexistent/path/that/does/not/exist"]);
...
fn test_verbose_level_2_shows_config() {
    ...
    assert!(
        stdout.contains("Configuration:")
            || stdout.contains("path_start")
            || stdout.contains("compare_by"),
...
fn test_summary_statistics() {
    ...
    let (stdout, stderr, status) = run_fundoubler(&["--md5", temp_dir.path().to_str().unwrap()]);
...
"#,
    ...
    assert!(
        output_path.exists(),
        "Output file from config should be created"
    );
...
fn test_init_config_silent_suppresses_output() {
    ...
    let (stdout, stderr, status) =
        run_fundoubler(&["--init-config", config_path.to_str().unwrap(), "--silent"]);
    assert!(status.success(), "stderr: {}", stderr);
    assert!(
...
fn test_size_only_comparison_no_hashing() {
    ...
    let (stdout, stderr, status) = run_fundoubler(&[temp_dir.path().to_str().unwrap(), "--size"]);
    assert!(status.success(), "stderr: {}", stderr);
    assert!(
...
fn test_special_characters_in_filename() {
    ...
    assert!(
        stdout.contains("dup")
            || stdout.contains("file with spaces")
            || stdout.contains("file_with_underscores")
    );
}
